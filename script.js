...
const elements = {
  ...
  particleColorBar: document.querySelector("#particleColorBar"),
...
function extractPaletteFromImage(image) {
  ...
  state.particles.color = state.activeImageColor || state.palette[0] || "#161616";
  renderPalette(state.palette);
  renderTextColorPalette();
  renderParticleColorPalette();
  applyCurrentBackground();
...
function renderParticleColorPalette() {
  const swatches = [...new Set(state.palette)];
  elements.particleColorBar.innerHTML = "";
  ...
  swatches.forEach((color) => {
    ...
    swatch.dataset.color = color;
    swatch.dataset.code = color;
    if (state.particles.color.toUpperCase() === color.toUpperCase()) {
      swatch.classList.add("is-gradient-source");
      swatch.classList.add("is-revealed");
    }
    elements.particleColorBar.appendChild(swatch);
  });
}
...
function handleParticleColorClick(event) {
  const target = event.target.closest(".palette-swatch");
  ...
  state.particles.color = target.dataset.color;
  renderParticleColorPalette();
  renderParticles();
}
...
function renderParticles() {
  ...
  const particleColor = state.particles.color || state.activeImageColor || state.palette[0] || "#161616";
